...
class ESN(object):
    ...
    def __init__(self, nus: Union[List[int], List[List[int]]], nys: Union[List[int], List[List[int]]],
                 ...
                 ridge: float = 0.01, burnout_train: int = 0, state_transform: Optional[str] = None,
        ...
        self.burnout_train = burnout_train  # it is a good practice to discard predictions during the training
    ...
    def state_fn(self, x, z, identity_readouts=True):
        ...
        if self.leaky_factor > 0:
            x_next = (1 - self.leaky_factor) * x + self.leaky_factor * post_activation
        else:
